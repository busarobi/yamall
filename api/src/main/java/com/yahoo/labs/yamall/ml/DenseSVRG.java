...
/**
 * created by acutkosky on 8/17/2017
 */

@SuppressWarnings("serial")
public class DenseSVRG implements Learner {
    ...
    private boolean doUseReset = true;

    DenseSVRGLearner baseLearner = null;
    ...
    public DenseSVRG(int bits) {
        this.baseLearner = new FreeRex(bits);
        initialize(bits);
    }

    public DenseSVRG(int bits, DenseSVRGLearner baseLearner) {
    ...
    private double updateBatchGradient(Instance sample) {
        ...
        baseLearner.updateScalingVector(sample);
    ...
    private double updateSGDStep(Instance sample) {
        ...
        for(Int2DoubleMap.Entry entry : sample.getVector().int2DoubleEntrySet()) {
            ...
            keys[i] = key;
            double varianceReducedNegativeGrad_i = x_i*(negativeGrad - negativeGrad_prev);
    ...
    private void endBatchPhase() {
        ...
        baseLearner.setNegativeBatchGrad(negativeBatchGradient);
    }

    private void endSGDPhase() {

        baseLearner.lazyUpdate();
